...
extension UIButton{
    ...
    /// set back arrow depends on language
    func setBackArrow(){
        self.setImage(UIImage(named: "dashArrow-\(app_lang)"), for: .normal)
    }
    
    /// set side arrow depends on language
    func setSideArrow(color:UIColor){
        self.setImage(UIImage(named: "dashArrow-\(app_lang)"), for: .normal)
    ...
    /// set image in button depends on URL
    /// - Parameter img: the URL of the image
    func setIMG(img: String){
        self.contentMode = .scaleToFill
        self.setImage(placeHolderImage, for: .normal)
        let loading = Indicator.shared.createActivityIndicator()
        loading.center = self.center
        self.addSubview(loading)
        if let imgUrl = URL(string: img.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""){
            self.sd_setImage(with: imgUrl, for: .normal) { [self] (img, _, _, _) in
                if let img = img{
